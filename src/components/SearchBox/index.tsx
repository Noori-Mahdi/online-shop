...
const SearchBox = () => {
  ...
  return (
    <div
      className={`'rounded-sm flex items-center relative w-full md:bg-gray-300 md:rounded-tr-sm md:rounded-br-sm border-gray-500`}
    >
      <div className="md:w-8 md:h-8 w-7 h-7">
        <Button
          className={` justify-center items-center rounded-full ${showSearchBox ? 'hidden md:flex md:rounded-none md:rounded-tl-sm md:rounded-bl-sm' : 'flex md:rounded-sm'} md:text-base text-sm font-bold`}
          label={<FiSearch />}
          type="button"
          onClick={() => {
            if (showSearchBox) {
      ...
      {showSearchBox && (
        <div className="grow fixed px-3 md:px-0 flex rounded-sm left-0 sm:px-3 top-0 mt-18 w-full md:mt-0 md:relative ">
          <div
